...
- Верхняя панель совпадает со стилем Welcome: `AppLogo` + индикатор `RU`.
...
- Перед результатом показывается промежуточное состояние `Analyzing`, как требуется в ТЗ.
- Если результат ещё считается на бэкенде, `QuestionnaireCubit` выполняет polling `GET /session` до готового `compatibility`.
